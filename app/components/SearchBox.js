  ...
  render(){
    return(
      <View style={{ flex :1 , width: '100%', backgroundColor:colors.white, borderRadius: 50, flexDirection : 'row', paddingTop: 5, paddingBottom: 5, paddingLeft: 15, paddingRight:15}}>
        ...
        <TextInput placeholder={(this.props.placeholder) ? this.props.placeholder : 'Search'} style={{width : '90%', marginLeft: 10}} placeholderTextColor="black" onChangeText = { (text) => (this.props.onChangeText) ? this.props.onChangeText(text) : null} onSubmitEditing={ (this.props.onSubmitEditing) ? this.props.onSubmitEditing : null} returnKeyType='search' />
      </View>
    )
  }
